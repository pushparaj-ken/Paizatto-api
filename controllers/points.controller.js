...
const pointsdetails = catchAsync(async (req, res) => {
    points.find({}).lean().exec().then((Result) => {
...
const addpoints = catchAsync(async (req, res) => {
    let values = req.body;
    if (values.customerUsername != '' && values.customerUsername != null && values.customerUsername != undefined

    ) {
        var Previouspoints = await points.findOne().sort('-id').lean().exec(); 
        let id = 1;
        if(Previouspoints.hasOwnProperty('id')){
...
const Updatepoints = catchAsync(async (req, res) => {
    let values = req.body;
    ...
    res.send({
        code:201,
        success: false,
        message:"Id required to update Points.",
        data:{},
        timestamp: new Date()
    });
}
...
const deletepoints = catchAsync(async (req, res) => {
    let values = req.body;
    let query = values;
    let changes = {
        $set: {
